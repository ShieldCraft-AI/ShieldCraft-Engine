...
def finalize_checklist(engine, partial_result=None, exception=None):
                    ...
                    if engine is not None and getattr(engine, 'checklist_context', None):
                        try:
                            engine.checklist_context.record_event(
                                "G13_PERSONA_OUTCOME_OVERRIDE_ATTEMPT",
                                "finalize",
                                "DIAGNOSTIC",
                                message="persona attempted to override outcome fields",
                                evidence={
                                    "event": ev})
                        except Exception:
                            pass
        ...
        try:
            ...
            try:
                mask_gates = {
                    ...
                    "G15_DISALLOWED_SELFHOST_ARTIFACT",
                    "G20_QUALITY_GATE_FAILED"}
                if (outcome or '').upper() == 'REFUSAL' and gid in mask_gates:
                    diag = {
                        ...
                        'meta': {
                            ...
                            'phase': phase,
                            'source': 'diagnostic',
                            'justification': 'refusal_masking_explanation',
    ...
    if exception is not None:
        try:
            ...
            items.append({'ptr': '/', 'text': f"internal_exception: {err_text}",
                         'severity': 'high', 'meta': {'exception': err_text}})
    ...
    try:
        ...
        try:
            if engine is not None and getattr(engine, 'checklist_context', None):
                try:
                    engine.checklist_context.record_event(
                        "G_INTERNAL_DERIVATION_FAILURE",
                        "finalize",
                        "DIAGNOSTIC",
                        message="derive_primary_outcome raised",
                        evidence={
                            "error": str(e)})
                except Exception:
                    pass
    ...
    try:
        ...
        tier_a_items = [
            it for it in checklist.get(
                'items', []) if (
                it.get('meta') or {}).get('source') == 'default' and (
                it.get('meta') or {}).get('tier') == 'A']
        if tier_a_items:

            has_blocker = any((ev.get('gate_id') or '').startswith('G_SYNTHESIZED_DEFAULT_')
                              and (ev.get('outcome') or '').upper() == 'BLOCKER' for ev in events)

            has_diag = any((ev.get('gate_id') or '').startswith('G_SYNTHESIZED_DEFAULT_')
                           and (ev.get('outcome') or '').upper() == 'DIAGNOSTIC' for ev in events)
    ...
    if derived.get('refusal'):
        ...
        try:
            non_persona_ev = next(e for e in events if (e.get('outcome') or '').upper()
                                  == 'REFUSAL' and not e.get('persona_id'))
            ...
            try:
                ...
                assert isinstance(refusal_meta.get('authority'), str) and refusal_meta.get(
                    'authority'), 'REFUSAL authority missing or invalid'
...
def _assert_semantic_invariants(checklist_obj, primary, gate_outcomes=None):
        ...
        if primary != 'ACTION':
            if roles.count('PRIMARY_CAUSE') != 1:
                raise AssertionError(
                    'Semantic invariant violated: exactly one PRIMARY_CAUSE required for non-ACTION outcomes')
        else:
    ...
    if primary == 'DIAGNOSTIC':
        if any(
                ((it.get('meta') or {}).get('gate') and (
                    'BLOCKER' in gate_outcomes.get(
                (it.get('meta') or {}).get('gate'),
                set()) or 'REFUSAL' in gate_outcomes.get(
                    (it.get('meta') or {}).get('gate'),
                set()))) for it in items_local):
            raise AssertionError(
                'Semantic invariant violated: DIAGNOSTIC outcome must not contain BLOCKER or REFUSAL items')
...
class Engine:
                ...
                if getattr(self, 'checklist_context', None):
                    try:
                        ...
                        record_refusal_event(
                            self.checklist_context,
                            "G1_ENGINE_READINESS_FAILURE",
                            "preflight",
                            message="engine readiness failure",
                            evidence={
                                "error": str(e)},
                            justification="engine_readiness_failure")
                    except Exception:
                        pass
            except Exception:
                pass
    ...
    def preflight(self, spec_or_path):
        ...
        try:
                ...
                if getattr(self, 'checklist_context', None):
                    try:
                        ...
                        record_refusal_event(
                            ...
                            "G2_GOVERNANCE_PRESENCE_CHECK",
                            "preflight",
                            message="governance presence check failed",
                            evidence={
                                "error": str(e)},
                            trigger="missing_authority",
                            scope="/governance",
                            justification="governance_presence_failed")
                    except Exception:
                        pass
            except Exception:
                pass
            ...
            try:
                if getattr(self, 'checklist_context', None):
                    try:
                        self.checklist_context.record_event(
                            "G2_GOVERNANCE_PRESENCE_CHECK",
                            "preflight",
                            "REFUSAL",
                        ...
                        if getattr(self, 'checklist_context', None):
                            try:
                                ...
                                record_refusal_event(
                                    ...
                                    "G3_REPO_SYNC_VERIFICATION",
                                    "preflight",
                                    message="repo sync missing",
                                    evidence={
                                        "error": str(e)},
                                    trigger="missing_authority",
                                    justification="repo_sync_missing")
                            except Exception:
                                pass
                    except Exception:
                        pass
                    ...
                    if getattr(self, 'checklist_context', None):
                        try:
                            self.checklist_context.record_event(
                                ...
                                "preflight",
                                "REFUSAL",
                                message="repo sync verification failed",
                                evidence={
                                    "error": str(e)})
                        except Exception:
                            pass
        ...
        try:
            ...
            try:
                if getattr(self, 'checklist_context', None):
                    try:
                        self.checklist_context.record_event(
                            "G6_VERIFICATION_SPINE_FAILURE",
                            "preflight",
                            "DIAGNOSTIC",
                            message="verification spine failure",
                            evidence={
                                "error": str(e)})
                    except Exception:
                        pass
        ...
        try:
            ...
            if enforce_flag or spec_enforce:
                from shieldcraft.services.validator.tests_attached_validator import (
                    verify_tests_attached, ProductInvariantFailure)
                ...
                try:
                    checklist_preview = self.checklist_gen.build(
                        spec, ast=ast_local, dry_run=True, run_test_gate=False, engine=self)
                except Exception:
            ...
            try:
                if getattr(self, 'checklist_context', None):
                    try:
                        self.checklist_context.record_event(
                            "G8_TEST_ATTACHMENT_CONTRACT",
                            "preflight",
                            "REFUSAL",
                            message="test-attachment contract failed",
                            evidence={
                                "error": str(e)})
                    except Exception:
                        pass
        ...
        if not isinstance(spec, dict):
            ...
            try:
                if getattr(self, 'checklist_context', None):
                    try:
                        self.checklist_context.record_event(
                            "G5_VALIDATION_TYPE_GATES", "preflight", "REFUSAL", message="spec not a dict")
                    except Exception:
                        pass
            except Exception:
                pass
        ...
        if self._last_validated_spec_fp != compute_spec_fingerprint(spec):
            try:
                if getattr(self, 'checklist_context', None):
                    try:
                        self.checklist_context.record_event(
                            "G5_VALIDATION_TYPE_GATES", "preflight", "REFUSAL", message="validation not recorded")
                    except Exception:
                        pass
            except Exception:
                pass
    ...
    def run(self, spec_path):
            ...
            if getattr(self, "_last_validated_spec_fp", None) != fp:
                try:
                    if getattr(self, 'checklist_context', None):
                        try:
                            self.checklist_context.record_event(
                                "G5_VALIDATION_TYPE_GATES", "preflight", "REFUSAL", message="validation not performed")
                        except Exception:
                            pass
                except Exception:
                    pass
        ...
        try:
            ...
            try:
                if getattr(self, 'checklist_context', None):
                    try:
                        self.checklist_context.record_event(
                            "G22_EXECUTE_INTERNAL_ERROR_RETURN", "generation", "DIAGNOSTIC", message=str(e))
                    except Exception:
                        pass
            except Exception:
                pass
        ...
        try:
            ...
            try:
                from shieldcraft.observability import emit_state
                emit_state(self, "readiness", "readiness", "ok" if readiness.get(
                    "ok") else "fail", str(readiness.get("results")))
            except Exception:
                pass
        except Exception:
        ...
        try:
            ...
            try:
                if getattr(self, 'checklist_context', None):
                    try:
                        self.checklist_context.record_event(
                            "G22_CODEGEN_INTERNAL_ERROR_RETURN", "generation", "DIAGNOSTIC", message=str(e))
                    except Exception:
                        pass
            except Exception:
                pass
    ...
    def run_self_host(self, spec, dry_run=False, emit_preview=None):
        ...
        try:
                ...
                if dry_run:
                    try:
                        if getattr(self, 'checklist_context', None):
                            try:
                                self.checklist_context.record_event(
                                    "G_VALIDATION_FAILURE", "preflight", "DIAGNOSTIC",
                                    message=f"Spec validation failed: {str(e)}")
                            except Exception:
                                pass
                    except Exception:
                        pass
                ...
                if getattr(self, "_last_validated_spec_fp", None) != fp:
                    try:
                        if getattr(self, 'checklist_context', None):
                            try:
                                self.checklist_context.record_event(
                                    "G5_VALIDATION_TYPE_GATES", "preflight", "REFUSAL",
                ...
                if dry_run:
                    try:
                        if getattr(self, 'checklist_context', None):
                            try:
                                self.checklist_context.record_event(
                                    "G_BUILD_FAILURE", "build", "DIAGNOSTIC", message=f"AST build failed: {str(e)}")
                            except Exception:
                                pass
                    except Exception:
                        pass
            ...
            try:
                ...
                if dry_run:
                    checklist = {
                        'items': [{'ptr': '/', 'text': f'Checklist generation failed: {str(e)}', 'severity': 'high'}]}
                else:
                    raise
            ...
            from shieldcraft.services.selfhost import is_allowed_selfhost_input, provenance_header
                    ...
                    if getattr(self, 'checklist_context', None):
                        try:
                            ...
                            record_refusal_event(
                                ...
                                justification="disallowed_selfhost_input",
                                trigger="missing_authority")
                        except Exception:
                            pass
                except Exception:
                    ...
                    if getattr(self, 'checklist_context', None):
                        try:
                            ...
                            record_refusal_event(
                                ...
                                "G14_SELFHOST_INPUT_SANDBOX",
                                "post_generation",
                                message="self-host not ready",
                                justification="selfhost_not_ready",
                                trigger="missing_authority")
                        except Exception:
                            pass
                except Exception:
                ...
                if not _is_worktree_clean():
                    try:
                        if getattr(self, 'checklist_context', None):
                            try:
                                self.checklist_context.record_event(
                                    "G14_SELFHOST_INPUT_SANDBOX", "post_generation",
                                    "REFUSAL", message="worktree not clean")
                            except Exception:
                                pass
                    except Exception:
                        pass
            ...
            try:
                manifest = {
                    ...
                    "spec_metadata": spec.get(
                        "metadata",
                        {}),
                    "bootstrap_items": len(bootstrap_items),
                    "codegen_bundle_hash": codegen_result.get(
                        "codegen_bundle_hash",
                        "unknown"),
                    "outputs": [
                        out["path"] for out in codegen_result.get(
                            "outputs",
                            [])],
                    "provenance": {
                        "engine_version": __import__(
                            'shieldcraft.services.selfhost',
                            fromlist=['ENGINE_VERSION']).ENGINE_VERSION,
                        "spec_fingerprint": fingerprint,
                        ...
                        if getattr(self, 'checklist_context', None):
                            try:
                                ...
                                record_refusal_event(
                                    ...
                                    "G15_DISALLOWED_SELFHOST_ARTIFACT",
                                    "post_generation",
                                    message=f"disallowed_selfhost_artifact: {rel_path}",
                                    evidence={
                                        "path": rel_path},
                                    justification="disallowed_selfhost_artifact",
                                    trigger="missing_authority",
                                    scope=f"path:{rel_path}")
                            except Exception:
                                pass
                    except Exception:
                        pass
            ...
            try:
                ...
                if violations:
                    ...
                    try:
                        if getattr(self, 'checklist_context', None):
                            try:
                                self.checklist_context.record_event(
                                    "G16_MINIMALITY_INVARIANT_FAILED", "post_generation",
                                    "REFUSAL", message="minimality invariant failed")
                            except Exception:
                                pass
                    except Exception:
                        pass
                ...
                manifest['checklist_execution_plan'] = {
                    'ordered_item_count': len(
                        plan.get(
                            'ordered_item_ids', [])), 'cycle_groups': plan.get(
                        'cycles', {}), 'missing_artifacts': plan.get(
                        'missing_artifacts', []), 'priority_violations': plan.get(
                        'priority_violations', [])}
                ...
                if plan.get('cycles'):
                    try:
                        if getattr(self, 'checklist_context', None):
                            try:
                                self.checklist_context.record_event(
                                    "G17_EXECUTION_CYCLE_DETECTED", "post_generation",
                                    "REFUSAL", message="execution cycle detected")
                            except Exception:
                                pass
                    except Exception:
                        pass
                ...
                if plan.get('missing_artifacts'):
                    try:
                        if getattr(self, 'checklist_context', None):
                            try:
                                self.checklist_context.record_event(
                                    "G18_MISSING_ARTIFACT_PRODUCER", "post_generation",
                                    "REFUSAL", message="missing artifact producer")
                            except Exception:
                                pass
                    except Exception:
                        pass
                ...
                if plan.get('priority_violations'):
                    try:
                        if getattr(self, 'checklist_context', None):
                            try:
                                self.checklist_context.record_event(
                                    "G19_PRIORITY_VIOLATION_DETECTED", "post_generation",
            ...
            try:
                if getattr(self, 'checklist_context', None):
                    try:
                        self.checklist_context.record_event(
                            "G14_SELFHOST_INTERNAL_ERROR_RETURN", "self_host", "DIAGNOSTIC", message=str(e))
                    except Exception:
                        pass
            except Exception:
                pass
    ...
    def run_self_build(self, spec_path: str = "spec/se_dsl_v1.spec.json", dry_run: bool = False):
        ...
        import shutil
        from pathlib import Path
        from shieldcraft.services.selfhost import (
            SELFBUILD_OUTPUT_DIR, SELFBUILD_BASELINE_DIR, DEFAULT_BASELINE_NAME,
            SELFBUILD_BITWISE_ARTIFACTS, is_allowed_diff)
    ...
    def verify_self_build_output(self, output_dir: str) -> None:
        """Verify that the self-build output snapshot matches the _current repo.
        ...
        _current = generate_snapshot(os.getcwd())
        if _current.get("tree_hash") != emitted.get("tree_hash"):
            raise RuntimeError("selfbuild_mismatch: emitted snapshot does not match _current repo snapshot")
    ...
    def execute(self, spec_path):
        ...
        if os.path.exists(prev_spec_path):
            ...
            with open(prev_spec_path, encoding='utf-8') as f:
        ...
        if isinstance(checklist_data, dict) and "items" in checklist_data:
            ...
            try:
                if getattr(self, 'checklist_context', None):
                    try:
                        self.checklist_context.record_event(
                            "G22_EXECUTE_INTERNAL_ERROR_RETURN",
                            "generation",
                            "DIAGNOSTIC",
            ...
            return finalize_checklist(
                ...
                partial_result={
                    "type": "internal_error",
                    "details": "Invalid checklist format"})
