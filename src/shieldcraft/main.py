...
def run_self_host(spec_file, schema_path, emit_preview=None, dry_run=False):
    ...
    try:
        ...
        if pre_items:
            from shieldcraft.services.guidance.checklist import (
                ...
                enrich_with_confidence_and_evidence, ensure_item_fields
            )
            ...
            try:
                with open(os.path.join(output_dir, "checklist_draft.json"), "w", encoding='utf-8') as cf:
        ...
        if ValidationError is not None and isinstance(e, ValidationError):
            ...
            with open(error_path, "w", encoding='utf-8') as f:
                ...
                json.dump({"errors": [str(e)]}, f, indent=2, sort_keys=True)
            ...
            try:
                ...
                manifest["state_reason"] = getattr(engine, "conversion_state_reason", None) or getattr(e, "code", None)
            ...
            try:
                ...
                try:
                    ...
                    interp = []
                checklist_preview = ChecklistGenerator().build(spec, ast=ast, dry_run=True, run_test_gate=False,
                                                               engine=engine, interpreted_items=interp)
                manifest["checklist_preview_items"] = len(checklist_preview.get(
                    "items", [])) if isinstance(checklist_preview, dict) else None
                ...
                try:
                    ...
                    manifest["checklist_preview_explanation"] = checklist_preview_explanation(
                        manifest.get("checklist_preview_items"), manifest.get("conversion_state"))
                except (ImportError, ValueError, TypeError, AttributeError):
                    pass
                # Attach annotated preview items when available (advisory only)
                try:
                    from shieldcraft.services.guidance.checklist import (
                        annotate_items, annotate_items_with_blockers, checklist_summary
                    )
                    if isinstance(checklist_preview, dict):
                        ...
                        try:
                            ...
                            for uit in interpreted_items_local:
                                ...
                                preview_items.append({
                                    ...
                                    "text": uit.get('claim'),
                                    "value": uit.get('claim'),
                                    "confidence": uit.get('confidence', 'low')
                                })
                            ...
                            try:
                                ...
                                with open(p, 'w', encoding='utf8') as sf:
                                    json.dump({'sufficiency': {'ok': False, 'mandatory_total': 0, 'mandatory_full': 0, 'mandatory_missing': 0,
                                              'mandatory_partial': 0, 'missing': [], 'partial': []}}, sf, indent=2, sort_keys=True)
                                manifest['checklist_sufficiency'] = {
                                    'ok': False, 'mandatory_total': 0, 'mandatory_full': 0, 'mandatory_missing': 0, 'mandatory_partial': 0, 'missing': [], 'partial': []}
                        ...
                        manifest["checklist_summary"] = checklist_summary(
                            preview_items, manifest.get("conversion_state"))
                        ...
                        try:
                            def _compute_first_safe(preview_items):
                                ...
                                p0_nonblocking = [it for it in preview_items if it.get(
                                    'priority') == 'P0' and not it.get('blocking')]
                                p0_blocking = [it for it in preview_items if it.get(
                                    'priority') == 'P0' and it.get('blocking')]
                                ...
                                if chosen in p0_nonblocking:
                                    ...
                                    if chosen.get('risk_if_false') == 'unsafe_to_act' or (
                                            chosen.get('severity') or '').lower() in ('high', 'critical'):
                                        risk = 'high'
                                        ...
                                        risk = 'low'
                                    rationale = "Evidence: " + \
                                        (repr(quote[:200]) if quote else ("ptr=" + (ptr or 'unknown')))
                                    ...
                                    return {"first_safe_action": {"action": chosen.get('action') or chosen.get(
                                        'text') or chosen.get('claim'), "rationale": rationale, "risk": risk, "why_this_first": why}}
                                ...
                                return {"refusal_action": {
                                    "action": f"Refuse to proceed until requirement is resolved: {chosen.get('action') or chosen.get('text')}", "explanation": reason}}
                        except (ValueError, TypeError, AttributeError):
                            def _compute_first_safe(_): return None
                        try:
                            ...
                            pass
                        try:
                            manifest["checklist_preview_summary"] = [{"id": it.get("id"), "claim": it.get(
                                "text") or it.get("value") or it.get("claim")} for it in preview_items[:5]]
                        except (ValueError, TypeError, AttributeError):
                            pass
                        ...
                        try:
                            ...
                            try:
                                ...
                                with open(cdpath, encoding='utf-8') as f:
                                    payload = json.load(f)
                                payload["items"] = ensure_item_fields(payload.get("items", []))
                                with open(cdpath, "w", encoding='utf-8') as cf:
                                    json.dump(payload, cf, indent=2, sort_keys=True)
                            except (ImportError, ValueError, TypeError, AttributeError, IOError, OSError):
                                pass
                        except (ValueError, TypeError, AttributeError):
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    rtxt = spec.get('metadata', {}).get('source_material') or spec.get(
                                        'raw_input') or json.dumps(spec, sort_keys=True)
                                ...
                                except (ValueError, TypeError, ImportError, IOError, OSError):
                                    reqs = []
                        ...
                        try:
                            from shieldcraft.services.guidance.checklist import annotate_items_with_remediation
                            cdpath = os.path.join(output_dir, "checklist_draft.json")
                            if os.path.exists(cdpath):
                                ...
                                # NOTE: implement build_spec_feedback or import it
                                # manifest["spec_feedback"] = feedback
                        except (ValueError, TypeError, AttributeError, ImportError, IOError, OSError):
                            pass
                        ...
                        try:
                            ...
                            trace = annotate_items_with_readiness_impact(
                                manifest.get("checklist_preview") or [], readiness, spec)
                            ...
                            if trace:
                                ...
                                blocker_items = sorted({iid for g, v in trace.items() if v.get("blocking")
                                                       for iid in v.get("item_ids", [])})
                                manifest["readiness_blockers_count"] = sum(
                                    1 for v in trace.values() if v.get("blocking"))
            ...
            if getattr(e, "code", None):
                missing_next.append({"code": e.code, "location": e.location,
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        comp_report = {'total_must': summary.get('total_requirements', 0), 'covered_must': summary.get('complete_count', 0), 'uncovered_must': [r for r in [
                            getattr(rr, 'requirement_id', None) for rr in results] if r is not None], 'weak_must': [], 'complete': (summary.get('complete_pct', 0.0) == 1.0)}
                        try:
                ...
                try:
                    ...
                    with open(p, 'w', encoding='utf8') as sf:
                        json.dump({'sufficiency': {'ok': False, 'mandatory_total': 0, 'mandatory_full': 0, 'mandatory_missing': 0,
                                  'mandatory_partial': 0, 'missing': [], 'partial': []}}, sf, indent=2, sort_keys=True)
                    manifest['checklist_sufficiency'] = {'ok': False, 'mandatory_total': 0, 'mandatory_full': 0,
                                                         'mandatory_missing': 0, 'mandatory_partial': 0, 'missing': [], 'partial': []}
                    manifest['checklist_sufficient'] = False
                except (IOError, OSError, TypeError):
                    pass
                ...
                try:
                    ...
                    with open(p, 'w', encoding='utf8') as sf:
                        json.dump({'requirements': [], 'summary': {'total_requirements': 0, 'complete_count': 0,
                                  'partial_count': 0, 'unbound_count': 0, 'complete_pct': 0.0}}, sf, indent=2, sort_keys=True)
            ...
            try:
                ...
                if os.path.exists(prev_spec_path):
                    with open(prev_spec_path, 'r', encoding='utf-8') as pf:
                        previous_spec = json.load(pf)
                else:
                    previous_spec = None
                spec_evolution = compute_evolution(previous_spec, spec)
            except (ValueError, TypeError, AttributeError, ImportError, IOError, OSError, json.JSONDecodeError):
                spec_evolution = None
            ...
            with open(summary_path, "w", encoding='utf-8') as f:
                ...
                try:
                    ...
                    prog = compute_progress_summary(prev, manifest.get("conversion_state"),
                                                    None, missing_next, None, manifest.get("spec_fingerprint"))
                except (ImportError, ValueError, TypeError, AttributeError, IOError, OSError):
                    prev = None
                    prog = None
                ...
                try:
                    def _compute_suppressed(output_dir, pre_scan_list):
                        ...
                        for sig in pre_scan_list or []:
                            entry = {"category": sig.get("intent_category") or "misc", "source_excerpt_hash": sig.get(
                                "excerpt_hash"), "text_excerpt": sig.get("text")}
                            mapped = None
                    ...
                    try:
                        if original_pre_scan:
                            ...
                            if suppressed:
                                with open(sup_path, "w", encoding='utf-8') as sf:
                ...
                try:
                    ...
                    exc = build_execution_preview(manifest.get("conversion_state"), manifest.get(
                        "checklist_preview") or [], None, missing_next)
                except (ValueError, TypeError, AttributeError, ImportError):
                    exc = None
                try:
                    from shieldcraft.services.guidance.artifact_contract import build_artifact_contract_summary
                    ac = build_artifact_contract_summary(manifest.get("conversion_state"), manifest.get(
                        "spec_metadata", {}).get("artifact_contract"), manifest.get("checklist_preview") or [], exc)
                except (ValueError, TypeError, AttributeError, ImportError):
                    ac = None
                summary = {
                    "status": "fail",
                ...
                if not os.path.exists(fb_path):
                    ...
                    def _build_spec_feedback_local(items, spec):
                        missing_sections = sorted(
                            [k for k in ("sections", "invariants", "metadata", "model") if not (spec or {}).get(k)])
                        weak = []
                        hints = []
                        for it in items:
                            conf = (it.get("confidence") or "medium")
                            if conf != "high":
                                weak.append({"id": it.get("id"), "confidence": conf,
                                            "inferred_from_prose": it.get("inferred_from_prose", False)})
                                ptr = it.get("ptr") or ""
                        ...
                        return {"missing_sections": missing_sections, "weakly_specified_intents": sorted(weak, key=lambda x: x.get(
                            "id") or ""), "suggested_next_edits": uniq_hints, "remediation_hints_count": len(hints)}

                    fb = _build_spec_feedback_local(items, spec)
                    with open(fb_path, "w", encoding='utf-8') as sf:
                        json.dump(fb, sf, indent=2, sort_keys=True)
                    manifest["spec_feedback"] = fb
            except (ValueError, TypeError, AttributeError, IOError, OSError, json.JSONDecodeError):
                pass
                ...
                try:
                    ...
                    subprocess.run(
                        [
                            sys.executable,
                            '-c',
                            'from shieldcraft.sufficiency.evaluator import evaluate_from_files, write_sufficiency_report; write_sufficiency_report(evaluate_from_files(".selfhost_outputs"), outdir=".selfhost_outputs")'],
                        check=False)
            ...
            # Enforce primary artifact invariant: exactly one of
            # `checklist_draft.json` or `refusal_report.json` must exist
            cd = os.path.join(output_dir, "checklist_draft.json")
            rr = os.path.join(output_dir, "refusal_report.json")
                    ...
                    if getattr(engine, 'checklist_context', None):
                        try:
                            engine.checklist_context.record_event(
                                "G15_DISALLOWED_SELFHOST_ARTIFACT", "post_generation", "REFUSAL", message="primary artifact invariant violated")
                        except (AttributeError, TypeError, ValueError):
                            pass
                except (AttributeError, TypeError):
                    pass
                raise RuntimeError(
                    "primary_artifact_invariant_violation: exactly one of checklist_draft.json or refusal_report.json must be present") from None
            ...
            if has_cd:
                ...
                if len(items) == 0:
                    ...
                    with open(os.path.join(output_dir, "silence_justification.json"), "w", encoding='utf-8') as sf:
        ...
        if SnapshotError is not None and isinstance(e, SnapshotError):
            ...
            with open(error_path, "w", encoding='utf-8') as f:
                ...
                json.dump({"errors": [{"message": str(e)}]},
    ...
    try:
        ...
        try:
            ...
            def _compute_first_safe_from_items(_): return None
        try:
        ...
        try:
            ...
            if os.path.exists(cl_path):
                ...
                items = manifest_data.get('checklist', []).get('items', []) if isinstance(
                    manifest_data.get('checklist'), dict) else manifest_data.get('checklist', [])
                ...
                if p0_violations:
                    try:
                        if getattr(engine, 'checklist_context', None):
                            try:
                                engine.checklist_context.record_event(
                                    "G20_QUALITY_GATE_FAILED", "post_generation", "REFUSAL", message="quality gate failed: p0 violations")
                ...
                if total == 0:
                    ...
                    try:
                        if getattr(engine, 'checklist_context', None):
                            try:
                                engine.checklist_context.record_event(
                                    "G20_QUALITY_GATE_FAILED", "post_generation", "REFUSAL", message="quality gate failed: zero items")
                            except (AttributeError, ValueError, TypeError):
                                pass
                    except (AttributeError, TypeError):
                        pass
                ...
                if ratio > 0.10:
                    try:
                        if getattr(engine, 'checklist_context', None):
                            try:
                                engine.checklist_context.record_event(
                                    "G20_QUALITY_GATE_FAILED", "post_generation", "REFUSAL", message="quality gate failed: low-signal ratio")
                            except (AttributeError, ValueError, TypeError):
                                pass
                    except (AttributeError, TypeError):
                        pass
                ...
                if total > 0 and inferred_all == total:
                    try:
                        if getattr(engine, 'checklist_context', None):
                            try:
                                engine.checklist_context.record_event(
                                    "G20_QUALITY_GATE_FAILED", "post_generation", "REFUSAL", message="quality gate failed: all items inferred from prose")
                            except (AttributeError, ValueError, TypeError):
                                pass
                    except (AttributeError, TypeError):
                        pass
            ...
            with open(manifest_path, "w", encoding='utf-8') as f:
                json.dump(manifest_data, f, indent=2, sort_keys=True)
        except (ImportError, ValueError, TypeError, AttributeError, IOError, OSError, RuntimeError):
            pass
        ...
        try:
            ...
            if os.path.exists(cl_path):
                ...
                if os.path.exists(reqp):
                    with open(cl_path, 'r', encoding='utf-8') as f:
                        items = json.load(f).get('items', [])
                    with open(reqp, 'r', encoding='utf-8') as f:
                ...
                try:
                    ...
                    with open(cl_path, 'r', encoding='utf-8') as f:
                        items = json.load(f).get('items', [])
                    cov = evaluate_spec_coverage(spec if isinstance(spec, dict) else {},
                                                 items, outdir=output_dir)
                    # persist root copy
                    try:
                        shutil.copyfile(os.path.join(output_dir, 'spec_coverage.json'),
                                        os.path.join('.selfhost_outputs', 'spec_coverage.json'))
                    except (IOError, OSError):
                        pass
                    manifest_data['spec_coverage_summary'] = {'total_units': cov.get(
                        'total_units'), 'covered_units': cov.get('covered_count')}
                    with open(manifest_path, "w", encoding='utf-8') as f:
                        json.dump(manifest_data, f, indent=2, sort_keys=True)
                except (ImportError, ValueError, TypeError, AttributeError, IOError, OSError):
                    pass
                ...
                try:
                    ...
                    # persist root copy
                    try:
                        shutil.copyfile(os.path.join(output_dir, 'implementability_verdict.json'),
                                        os.path.join('.selfhost_outputs', 'implementability_verdict.json'))
                    except (IOError, OSError):
                        pass
        ...
        try:
            ...
            if isinstance(checklist_obj, dict):
                ...
                annotate_items_with_blockers(checklist_obj.get("items", []),
                                             validation_errors=None, readiness_results=manifest_data.get("readiness"))
                try:
                    with open(os.path.join(output_dir, "checklist_draft.json"), "w", encoding='utf-8') as cf:
                        json.dump({"items": checklist_obj.get("items", []), "status": (
                            "ok" if (manifest_data.get("readiness") or {}).get("ok") else "draft")}, cf, indent=2, sort_keys=True)
                    emitted = True
        ...
        try:
            ...
            try:
                cpe = checklist_preview_explanation(manifest_data.get(
                    "checklist_preview_items"), manifest_data.get("conversion_state"))
            except (ValueError, TypeError, AttributeError, ImportError):
                cpe = None

            try:
                from shieldcraft.services.guidance.conversion_path import build_conversion_path
                conv = build_conversion_path(manifest_data.get("conversion_state"),
                                             md_missing, manifest_data.get("readiness"))
            except (ValueError, TypeError, AttributeError, ImportError):
                conv = None
            ...
            try:
                ...
                if checklist_obj and checklist_obj.get("items"):
                    annotate_items(checklist_obj.get("items"))
                    annotate_items_with_blockers(checklist_obj.get("items"), validation_errors=None, readiness_results=(
                        manifest_data.get("readiness") or checklist_obj.get("_readiness")))
                    try:
                        from shieldcraft.services.guidance.checklist import enrich_with_confidence_and_evidence, ensure_item_fields
                    ...
                    try:
                        ...
                        try:
                            ...
                            pass
                    except (ImportError, IOError, OSError, TypeError, json.JSONDecodeError, ValueError):
                        pass
                    ...
                    try:
                        ...
                        if not os.path.exists(cdpath):
                            ...
                            if isinstance(cobj, dict) and cobj.get("items"):
                                ...
                                annotate_items_with_blockers(cobj.get("items"), validation_errors=None, readiness_results=(
                                    manifest_data.get("readiness") or cobj.get("_readiness")))
                                try:
                                ...
                                with open(cdpath, "w", encoding='utf-8') as cf:
                                    json.dump({"items": cobj.get("items"), "status": ("ok" if (manifest_data.get(
                                        "readiness") or {}).get("ok") else "draft")}, cf, indent=2, sort_keys=True)
                    except (ValueError, TypeError, AttributeError, ImportError):
                        pass
                    emitted = True
                    status = ("ok" if (manifest_data.get("readiness") or {}).get("ok") else "draft")
                    ...
                    try:
                        ...
                        for it in items:
                            ...
                            if conf != "high":
                                ...
                                hints.append(h)
                        uniq_hints = sorted(set(hints))
                        feedback = {"missing_sections": missing_sections, "weakly_specified_intents": sorted(
                            weak, key=lambda x: x.get("id") or ""), "suggested_next_edits": uniq_hints, "remediation_hints_count": len(hints)}
                        with open(fb_path, "w", encoding='utf-8') as sf:
            ...
            try:
                ...
                try:
                    blocker_items = sorted({iid for g, v in (trace or {}).items() if v.get("blocking")
                                           for iid in v.get("item_ids", [])})
                ...
                if os.path.exists(rr):
                    ...
                    try:
                        ...
                        if strong:
                            ...
                            is_safe = "unknown"
                            why_not = "No items with sufficient confidence"
                    except (IOError, OSError, json.JSONDecodeError, ValueError, KeyError, TypeError, AttributeError):
                        pass
                summary["is_it_safe_to_act"] = is_safe
                summary["where_is_safe"] = where_safe
                summary["why_not_safe"] = why_not
            ...
            try:
                ...
                if not items and os.path.exists(os.path.join(output_dir, "checklist_draft.json")):
                    try:
                        items = json.load(open(os.path.join(output_dir, "checklist_draft.json"), encoding='utf-8')).get("items", []) or []
            ...
            try:
                ...
                def _compute_first_safe_summary(_): return None
            try:
            ...
            try:
                ...
                try:
                    ...
                    with open(summary_path, 'w', encoding='utf-8') as f2:
            ...
            try:
                ...
                if os.path.exists(sfile):
                    ...
                    try:
                        ...
                        with open(tmp2, 'w', encoding='utf-8') as sf:
                            json.dump(summary, sf, indent=2, sort_keys=True)
                        try:
                            os.replace(tmp2, summary_path)
                        except OSError:
                            with open(summary_path, 'w', encoding='utf-8') as sf2:
                                json.dump(summary, sf2, indent=2, sort_keys=True)
                    except (IOError, OSError, TypeError):
                        pass
            except (ValueError, TypeError, AttributeError, ImportError):
                pass
        ...
        if not dry_run:
            try:
                import subprocess
                import sys
                subprocess.run([sys.executable, '-c', 'import json; from shieldcraft.coverage.evaluator import evaluate_spec_coverage; m=json.load(open(".selfhost_outputs/manifest.json", \'r\', encoding=\'utf-8\')); s=m.get("spec_metadata",{}).get("source_material") or {}; items=json.load(open(".selfhost_outputs/checklist.json", \'r\', encoding=\'utf-8\')).get("items", []); evaluate_spec_coverage(s if isinstance(s, dict) else {}, items, outdir=".selfhost_outputs")'], check=False)
            except (OSError, ValueError, TypeError, ImportError):
                pass
        ...
        try:
            ...
            import sys
            subprocess.run(
                [sys.executable, '-c', 'from shieldcraft.verdict.aggregator import compute_implementability; compute_implementability(".selfhost_outputs")'], check=False)
        except (OSError, ValueError, TypeError, ImportError):
            pass
