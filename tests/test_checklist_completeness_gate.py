...
def test_litmus_passes_completeness():
    ...
    cl = json.load(open('.selfhost_outputs/checklist.json', encoding='utf-8'))
...
def test_synthetic_failure_reports_uncovered(tmp_path):
    ...
    if os.path.exists('.selfhost_outputs/completeness_report.json'):
        rep = json.load(open('.selfhost_outputs/completeness_report.json', encoding='utf-8'))
    else:
        # older compatibility artifact
        rep = json.load(open('.selfhost_outputs/requirement_completeness.json', encoding='utf-8'))
    # allow either shape: legacy completeness_report
    # (uncovered_must/uncovered) or requirement_completeness
    # (requirements/state)
    uncovered = None
    ...
    manifest = json.load(open('.selfhost_outputs/manifest.json', encoding='utf-8'))
