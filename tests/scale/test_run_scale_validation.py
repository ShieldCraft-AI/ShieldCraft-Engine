...
def test_run_scale_collects_metrics_and_enforces_non_silence(tmp_path):
    ...
    s1 = specs / "good.json"
    _make_spec(s1, {"metadata": {"product_id": "good", "spec_format": "canonical_json_v1", "self_host": True}, "model": {"version": "1.0"}, "sections": [{"id": "core"}]})
    ...
    try:
        ...
        report_path = run_scale(str(specs), 'src/shieldcraft/dsl/schema/se_dsl.schema.json', out_report=str(out), artifacts_root=str(artifacts), allow_dirty=True)
...
def test_scale_report_is_reproducible(tmp_path):
    ...
    s1 = specs / "one.json"
    s2 = specs / "two.json"
    _make_spec(s1, {"metadata": {"product_id": "one", "spec_format": "canonical_json_v1", "self_host": True}, "model": {"version": "1.0"}, "sections": [{"id": "a"}]})
    _make_spec(s2, {"metadata": {"product_id": "two", "spec_format": "canonical_json_v1", "self_host": True}, "model": {"version": "1.0"}, "sections": [{"id": "b"}]})
    ...
    try:
        p1 = run_scale(str(specs), 'src/shieldcraft/dsl/schema/se_dsl.schema.json', out_report=str(out1), artifacts_root=str(artifacts), allow_dirty=True)
        p2 = run_scale(str(specs), 'src/shieldcraft/dsl/schema/se_dsl.schema.json', out_report=str(out2), artifacts_root=str(artifacts), allow_dirty=True)
        b1 = open(p1, 'rb').read()
        b2 = open(p2, 'rb').read()
