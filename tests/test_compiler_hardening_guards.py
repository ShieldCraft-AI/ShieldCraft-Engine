def test_tier_enforcement_function_exists_and_returns_items():
    ...
    class StubContext:
        ...
        def record_event(self, gid, phase, outcome, message=None, evidence=None, **kwargs):
            self._events.append({'gate_id': gid, 'phase': phase, 'outcome': outcome,
    ...
    assert any(ev['gate_id'].startswith('G_TIER_A_MISSING') or ev['gate_id'].startswith('G_TIER_B_MISSING')
