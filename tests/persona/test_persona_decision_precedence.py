...
def test_conflicting_persona_advice_produces_diagnostic(monkeypatch):
    spec = {"metadata": {"product_id": "p", "version": "1.0"}, "sections": {"core": {"description": "x"}}}
    clear_registry()
    p1 = Persona(name="a1", scope=["checklist"], allowed_actions=["annotate"], constraints={"constraint": [{"match": {"ptr": "/sections"}, "set": {"meta": {"note": "a1"}}}]})
    p2 = Persona(name="a2", scope=["checklist"], allowed_actions=["annotate"], constraints={"constraint": [{"match": {"ptr": "/sections"}, "set": {"meta": {"note": "a2"}}}]})
    register_persona(p1)
    register_persona(p2)
    ...
    evs = engine.checklist_context.get_events()
    assert any(e.get("code") == "G15_PERSONA_CONSTRAINT_DISALLOWED" for e in evs) or any(it.get("meta", {}).get("persona_constraints_applied") for it in chk.get("items", []))
