...
def test_persona_veto_halts_execution_cleanly(tmp_path, monkeypatch):
    ...
    emit_veto(engine, persona, "preflight", "forbidden", {
              "explanation_code": "forbidden_reason", "details": "vetoed by persona"}, "high")
...
def test_multi_persona_resolution(tmp_path, monkeypatch):
    ...
    # Preflight now treats persona vetoes as advisory; ensure selection
    # resolves to persona b and an advisory event exists
    engine.checklist_context = StubCtx()
    res = engine.preflight(spec)
    assert res.get("ok") is True
