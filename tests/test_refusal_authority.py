def test_refusal_event_contains_authority_and_propagates():
    ...
    assert any(e.get('evidence', {}).get('refusal', {}).get('authority') ==
               'governance' for e in engine.checklist_context.get_events())
...
def test_paired_diagnostic_on_missing_authority():
    ...
    assert any(
        it.get(
            'text',
            '').startswith('MISSING_AUTHORITY:') or it.get(
            'text',
            '').startswith('REFUSAL_DIAG:') for it in items)
